    ...
    <form action="/products/update/<?php echo $product['id'] ?>" method="POST">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="mb-4">
                ...
                <input type="number" id="position" name="position" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="<?php echo htmlspecialchars($product['position']) ?>">
            </div>
            <div class="mb-6">
                <label for="status" class="block text-gray-700 text-sm font-bold mb-2">وضعیت</label>
                <select id="status" name="status" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                    <option value="available" <?php echo $product['status'] === 'available' ? 'selected' : '' ?>>موجود</option>
                    <option value="unavailable" <?php echo $product['status'] === 'unavailable' ? 'selected' : '' ?>>ناموجود</option>
                    <option value="draft" <?php echo $product['status'] === 'draft' ? 'selected' : '' ?>>پیش‌نویس</option>
                </select>
            </div>
        </div>
