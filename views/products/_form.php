...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <div class="mb-6">
        ...
        <select id="status" name="status" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
            <option value="available"   <?php echo (isset($product['status']) && $product['status'] === 'available') ? 'selected' : ''; ?>>موجود</option>
            <option value="unavailable" <?php echo (isset($product['status']) && $product['status'] === 'unavailable') ? 'selected' : ''; ?>>ناموجود</option>
            <option value="draft"       <?php echo (isset($product['status']) && $product['status'] === 'draft') ? 'selected' : ''; ?>>پیش‌نویس</option>
        </select>
    </div>
</div>
