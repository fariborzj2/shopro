            ...
            <?php foreach ($posts as $post): ?>
                <div class="p-4 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                    <div class="flex justify-between items-start mb-2">
                        <div class="flex-1">
                            <h3 class="text-sm font-bold text-gray-900 dark:text-white line-clamp-1"><?= htmlspecialchars($post['title']) ?></h3>
                            <span class="text-xs text-gray-500 dark:text-gray-400 mt-1 block"><?= htmlspecialchars($post['category_name'] ?? 'بدون دسته‌بندی') ?></span>
                        </div>
                         <?php
                    ...
                    <div class="flex justify-between items-center mt-3 pt-3 border-t border-gray-100 dark:border-gray-700">
                        ...
                        <div class="flex items-center gap-3">
                            <a href="<?php echo url('blog/posts/edit/' . $post['id']); ?>" class="text-indigo-600 dark:text-indigo-400 text-sm font-medium">ویرایش</a>
                    ...
                    <?php foreach ($posts as $post): ?>
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors group">
                            ...
                            <td class="px-6 py-4 text-sm">
                                 <?php
                                    $status_class = match($post['status']) {
                                        'published' => 'bg-emerald-50 text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-800',
                                        'draft' => 'bg-amber-50 text-amber-700 dark:bg-amber-900/20 dark:text-amber-400 border border-amber-100 dark:border-amber-800',
                            ...
                            <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">
                                <?= \jdate('Y/m/d', strtotime($post['created_at'])) ?>
                            </td>
