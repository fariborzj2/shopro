<h1 class="text-3xl font-bold mb-6">ویرایش نوشته: <?= htmlspecialchars($post['title']) ?></h1>
...
<div class="bg-white shadow-md rounded-lg p-8">
    <form action="/blog/posts/update/<?= $post['id'] ?>" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" id="title" name="title" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="<?= htmlspecialchars($post['title']) ?>" required>
        </div>

        <div class="mb-4">
            <label for="slug" class="block text-gray-700 text-sm font-bold mb-2">اسلاگ (Slug)</label>
            <input type="text" id="slug" name="slug" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="<?= htmlspecialchars($post['slug']) ?>" required>
        </div>
        ...
        <div class="mb-4">
            ...
            <textarea id="excerpt" name="excerpt" rows="3" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"><?= htmlspecialchars($post['excerpt']) ?></textarea>
        </div>

        <div class="mb-4">
            <label for="content" class="block text-gray-700 text-sm font-bold mb-2">محتوای اصلی</label>
            <textarea id="content" name="content" rows="10" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"><?= htmlspecialchars($post['content']) ?></textarea>
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="text" id="published_at_jalali" name="published_at_jalali" autocomplete="off"
                   value="<?= $post['published_at_jalali'] ?? '' ?>"
        ...
        <div class="mb-4">
            <label for="faq_items" class="block text-gray-700 text-sm font-bold mb-2">پرسش‌های متداول مرتبط:</label>
            <select id="faq_items" name="faq_items[]" multiple class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline h-32">
                <?php foreach ($all_faq_items as $faq): ?>
                    <option value="<?= $faq['id'] ?>" <?= in_array($faq['id'], $post_faq_items) ? 'selected' : '' ?>>
